---
name: error-hunter
description: 执行 AI 面试题的多轮、多模型、批量审题流程。适用于大批量 AI 生成面试题的质量审核，包括 40 题分批、是否删除判断、A/B/C 二级错误类型统计、一轮无错题进二轮、一轮/二轮/X轮错题进入重出池、重出题回流后进入三到N轮循环审核、审核进度看板、断点续跑和模型质量校准。用户提到审题、批量审题、题库审核、多轮审核、A/B/C类错误、删除原因统计、审核看板、进度可视化、保留题干重出答案、待重新出题时使用。
---

# Error Hunter

## 运行前必须读取的材料

执行任何审题任务前，必须先读取并对齐以下材料。这些文件定义了真实业务流程、审题提示词、结果汇总口径和输出字段要求。

- `/Users/risei/Desktop/北森-测评产品经理/AI面试官/专业能力/AI审题/AI审题流程与材料/AI审题流程与材料_3Apr26.xlsx`
- `/Users/risei/Desktop/北森-测评产品经理/AI面试官/专业能力/AI审题/AI审题流程与材料/题目审核流程要求提示词.txt`
- `/Users/risei/Desktop/北森-测评产品经理/AI面试官/专业能力/AI审题/AI审题流程与材料/审题提示词.txt`
- `/Users/risei/Desktop/北森-测评产品经理/AI面试官/专业能力/AI审题/AI审题流程与材料/第一轮审核结果汇总提示词.txt`
- `/Users/risei/Desktop/北森-测评产品经理/AI面试官/专业能力/AI审题/AI审题流程与材料/第二轮审核结果汇总提示词.txt`
- `/Users/risei/Desktop/北森-测评产品经理/AI面试官/专业能力/AI审题/AI审题流程与材料/三-N轮审核结果汇总提示词.txt`

## 什么时候使用这个 Skill

当用户需要审核一批 AI 生成的面试题时，使用本 Skill。典型场景包括：

- 用户需要审核一个或多个岗位的大批量面试题。
- 审核流程需要多个大模型并行审题。
- 审核不是一次完成，而是需要一轮、二轮、三轮到 N 轮逐步筛题。
- 用户需要统计每轮删除题目的错误类型、错误数量和典型例子。
- 用户希望看到每个模型、每个批次的审核进度，便于发现卡住、过严或过松的问题。

## 执行前必须确认的信息

开始审题前，必须向用户确认并复述以下信息：

1. 本次审核的行业和岗位，岗位可以是单个岗位，也可以是岗位列表。
2. 待审题表的文件路径。
3. 当前是第几轮审核，以及计划执行到第几轮。
4. 每轮使用的模型：
   - 第一轮默认使用 `GPT5.5`、`GPT5.4`、`Sonnet4.6`、`Opus4.7`、`Gemini3.1pro`。
   - 第二轮默认沿用第一轮 5 个模型，除非用户另行指定。
   - 第三轮到第 N 轮默认使用 `GPT5.5` 和 `Gemini3.1pro` 两个模型。
5. 每批题量，默认每批 `40` 题。
6. 审题提示词来源，优先使用本项目中的本地提示词文件。

如果以上信息缺失，不要直接开始执行，应先用简短问题向用户确认。

## 标准审题流程

### 第 1 步：准备批次

按岗位分别整理待审题表，并保持原始题目顺序稳定不变。

- 每 `40` 题为一批。
- 批次编号使用固定格式：`R{轮次}-B{三位批次号}`，例如 `R1-B001`。
- 建立主索引，至少保留以下信息：
  - `question_id`
  - `招聘场景` 或 `场景`
  - `行业`
  - `岗位` 或 `岗位名称`
  - `JD`
  - `batch_id`
  - `题目` 或 `问题`
  - `参考答案-第一层` 或 `第一层`
  - `参考答案-第二层` 或 `第二层`
  - `参考答案-第三层` 或 `第三层`
  - `难度`
  - `建议作答时长` 或 `建议作答时间`
  - `技能分类`
  - `技能`
  - `知识点`
  - `提示词`

输入题表和中间审核表可能使用不同字段名。执行时必须做字段映射，不要因为字段名从 `题目` 变成 `问题`、从 `岗位` 变成 `岗位名称`、从 `招聘场景` 变成 `场景` 就丢字段。

### 第 1.1 步：先做 Pilot Gate（质量校准）

不要一开始就让所有模型跑完全量。对于大批量任务，必须先让每个模型试跑 `1-2` 个批次，建议试跑 `2` 个批次，也就是最多 `80` 题。

试跑结束后，需要检查：

- 每个模型的删题率是否明显过高或过低。
- 每个模型的删题率与其他模型中位数相比是否偏差过大。
- 模型给出的删除理由是否符合审题提示词和错误分类口径。
- 模型保留题目的理由是否合理，是否存在明显漏审。

如果某个模型明显过松、过严或误解规则，应暂停该模型后续批次，先校准提示词，再重新跑 pilot。不要在 pilot 未通过时直接进入全量审核。

### 第 2 步：逐批审题

每个模型都必须独立审题，不能抽查，也不能只看部分题目。

模型配置按轮次执行：

- 第一轮：`GPT5.5`、`GPT5.4`、`Sonnet4.6`、`Opus4.7`、`Gemini3.1pro` 五个模型分别审题。
- 第二轮：默认继续使用上述五个模型，除非用户指定调整。
- 第三轮到第 N 轮：默认使用 `GPT5.5` 和 `Gemini3.1pro` 两个模型分别审题。

每个模型的审题结果必须在原表字段基础上追加以下字段：

- `是否删除`
- `错误类型`
- `删除原因`
- `疑似与肯定`
- `删除轮次`（第二轮及后续轮次必须填写）

字段填写要求：

- `是否删除` 只能填写 `是` 或 `否`。
- `错误类型` 优先填写二级错误类型，例如 `A1~A4`、`B1~B6`、`C1~C2`。如果原始结果无法判断二级类型，才退回到 `A/B/C`。
- `删除原因` 必须写清楚具体错在哪里，以及正确应该是什么；如果是重复题，按提示词要求可以不填删除原因。
- `疑似与肯定` 只能填写 `肯定有`、`疑似有`、`肯定无`、`疑似无`。
- `删除轮次` 填写当前审核轮次，例如 `2`、`3`；第一轮审核结果可以不填。

每完成一个批次，都要立即保存该批次结果，避免模型卡住后整轮重跑。

### 第 3 步：统一“疑似与肯定”和“是否删除”

不同模型可能对 `是否删除` 和 `疑似与肯定` 的标注不完全一致，因此每轮汇总前必须做标准化：

- `疑似与肯定` 为 `肯定有` 或 `疑似有` 时，统一视为 `是否删除 = 是`。
- `疑似与肯定` 为 `肯定无` 或 `疑似无` 时，统一视为 `是否删除 = 否`。

同一题在多模型汇总时，只要有任一模型标记为删除，本轮合并结果就视为有问题；如果所有模型都标记为不删除，本轮合并结果才视为无问题。

### 第 3.1 步：记录进度并支持断点续跑

当批次数很多时，必须记录每个模型、每个批次的执行状态。状态只能使用：

- `pending`
- `running`
- `done`
- `blocked`
- `failed`

如果模型卡住，需要：

1. 标记当前批次为 `blocked`。
2. 记录该模型最后一个完成的批次。
3. 给出下一步应从哪个批次继续。
4. 继续时只跑未完成批次，不要重复跑已经 `done` 的批次，除非用户明确要求重跑。

### 第 4 步：第一轮结束后的分流

第一轮结束后，先根据五个模型的标准化结果判断每道题是否有错。

- 第一轮无错题：所有模型都标记为 `否` 的题目，进入第二轮审核。
- 第一轮错题：任一模型标记为 `是` 的题目，进入错题重出池。用户拿到重出后的题目或答案后，这批题不再回到第二轮，而是进入第三轮到第 N 轮循环审核。

第一轮必须输出：

1. 一轮审核结果汇总。
2. 待二轮审核题表。
3. 一轮审核后需要重出的内容。
4. `round1_dashboard.html`。
5. `round1_dashboard.xlsx`。

待二轮审核题表只保留以下字段：

- `问题`
- `第一层`
- `第二层`
- `第三层`
- `难度`
- `建议作答时间`
- `行业`
- `岗位名称`
- `技能分类`
- `技能`
- `知识点`
- `场景`

### 第 5 步：第二轮结束后的分流

本 Skill 不负责执行“重新出题”或“重新出答案”。它只负责在审核后告诉用户哪些题需要重出答案，哪些题需要题目和答案都重出。

第二轮只审核第一轮完全无错的题。第二轮结束后：

- 第二轮无错题：题目直接保留，进入最终 `可保留题目答案`。
- 第二轮错题：结合第一轮和第二轮审核结果，输出到错题重出池。用户拿到重出后的题目或答案后，进入第三轮到第 N 轮循环审核。

错题重出池按以下规则分流：

- 仅出现 `B` 类错误的题目，进入 `rewrite_answer`，表示题干可以保留，只需要重新出参考答案。
- 出现 `A` 类或 `C` 类错误的题目，进入 `rewrite_question`，表示题目和答案都需要重新出。

第二轮必须输出：

1. 本轮多模型审核结果汇总。
2. 二轮审核后需要重出的内容。
3. 本轮可保留题目。
4. `round{轮次}_dashboard.html`。
5. `round{轮次}_dashboard.xlsx`。

重出内容优先输出为一个 `题库审核汇总.xlsx`，包含 `可保留题目答案`、`需要重新出题`、`保留题干重出答案` 三个 sheet。若用户或下游团队要求拆分文件，再从其中拆出 `保留题干重出答案_交付表.xlsx` 和 `待重新出题_交付表.xlsx`。

### 第 5.1 步：第三轮到第 N 轮循环审核

第三轮到第 N 轮的输入，是前序轮次错题重出后回流的新题目或新答案。

每一轮都按以下逻辑循环：

1. 用 `GPT5.5` 和 `Gemini3.1pro` 两个模型审核重出题。
2. 如果没有模型审出错误，题目进入 `可保留题目答案`。
3. 如果仍有错误，继续按 `rewrite_answer` 和 `rewrite_question` 分流，并交给用户或下游团队重出。
4. 用户拿到重出结果后，作为下一轮输入继续审核。
5. 当本轮仍需重出的题目少于 `10` 个时，提示用户可以进入人工判断，由用户决定是否继续重出。

### 第 5.2 步：每轮文件夹结构

第一轮使用以下文件夹：

- `待一轮审核题表`
- `审题结果`
- `一轮审核结果汇总`
- `一轮审核后需要重出的内容`

第二轮使用以下文件夹：

- `待二轮审核题表`
- `二轮审核结果`
- `二轮审核结果汇总`
- `二轮审核后需要重出的内容`

第三轮到第 N 轮使用以下文件夹：

- `待X轮审核题表`
- `X轮审核结果`
- `X轮审核结果汇总`
- `X轮审核后需要重出的内容`

### 第 6 步：人工最终检查

当本轮仍需重出的题目数量已经少于阈值，或用户要求人工判断时，需要提示用户做最终人工检查。人工检查重点包括：

- 题干表达是否通顺。
- 是否存在不适合面试口语表达的措辞。
- 是否存在语音播报困难的标点、符号、公式或代码片段。
- 是否需要继续重出，还是可以人工放行。

### 第 7 步：重出题回流后的下一轮审核

当用户从大数据组拿到重新出的题目或答案后，把回流表作为第三轮或更高轮次的输入继续审核。下一轮仍然遵守分批、pilot gate、断点续跑、看板输出和轮次分流规则。

## 每轮必须输出的内容

每一轮都必须输出基础产物：

1. 多模型审核结果汇总表。
2. 删除题目的错误统计，包括一级错误类型和二级错误类型。
3. 每个二级错误类型给出 2 个代表性例子。
4. 进度与质量看板。

看板只保留两个文件：

- `round{轮次}_dashboard.html`
- `round{轮次}_dashboard.xlsx`

不要单独输出细分错误统计看板，例如不要保留 `*_删除错误统计_细分.html`、`*_删除错误统计_细分.xlsx` 或 `*_删除错误统计_细分.md`。细分错误统计应嵌入 dashboard 内。

## 每轮汇总表字段规则

基础字段必须优先按样例文件保留为：

- `问题`
- `第一层`
- `第二层`
- `第三层`
- `难度`
- `建议作答时间`
- `行业`
- `岗位名称`
- `技能分类`
- `技能`
- `知识点`
- `场景`

如果原始输入是 `题目`、`参考答案-第一层`、`参考答案-第二层`、`参考答案-第三层`、`岗位`、`招聘场景`、`建议作答时长`，导出审核过程表时分别映射为 `问题`、`第一层`、`第二层`、`第三层`、`岗位名称`、`场景`、`建议作答时间`。

### 第一轮汇总表

第一轮汇总表保留基础字段，并追加 5 个模型的审核结果字段。每个模型追加 4 列：

- `{模型名}_是否删除`
- `{模型名}_错误类型`
- `{模型名}_删除原因`
- `{模型名}_疑似与肯定`

第一轮模型包括：

- `GPT5.5`
- `GPT5.4`
- `Sonnet4.6`
- `Opus4.7`
- `Gemini3.1pro`

### 第二轮汇总表

第二轮默认使用与第一轮相同的 5 个模型，除非用户明确指定其他模型。第二轮汇总表是在一轮审核结果表基础上更新，每个模型追加 5 列：

- `{模型名}_是否删除`
- `{模型名}_错误类型`
- `{模型名}_删除原因`
- `{模型名}_疑似与肯定`
- `{模型名}_删除轮次`

### 第三轮到第 N 轮汇总表

第三轮到第 N 轮默认使用 2 个模型：

- `GPT5.5`
- `Gemini3.1pro`

汇总表需要追加两个模型各自的 4 个字段，并增加 `合并` 字段。`合并` 字段表示两个模型的 `是否删除` 合并结果。

## 最终审核汇总文件

从第二轮开始，以及第三轮到第 N 轮每轮结束后，优先输出一个 `题库审核汇总.xlsx`。该文件包含三个 sheet：

1. `可保留题目答案`
2. `需要重新出题`
3. `保留题干重出答案`

三个 sheet 的字段参考样例文件，优先使用：

- `问题`
- `最终题目`
- `第一层`
- `第二层`
- `第三层`
- `技能分类`
- `技能`
- `知识点`
- `难度`
- `行业`
- `岗位名称`
- `场景`
- `一审_是否删除`
- `一审_删除原因`
- `二审_是否删除`
- `二审_删除原因`
- `题目是否修改`
- `是否重新出题`
- `一审_错误大类集合`
- `二审_错误大类集合`

第三轮到第 N 轮可以将 `一审/二审` 字段按实际轮次扩展为 `X轮_是否删除`、`X轮_删除原因`、`X轮_错误大类集合`，但不要改动用户明确提供的模板字段。

## 进度日志要求

每个模型、每个批次都要记录日志，字段包括：

- `round`
- `model`
- `batch_id`
- `status`
- `start_time`
- `end_time`
- `duration_sec`
- `questions_total`
- `delete_count`
- `keep_count`
- `error_A_count`
- `error_B_count`
- `error_C_count`
- `notes`

如果模型卡住，必须在日志中写入 `blocked`，并在 `notes` 中说明原因。

## 卡住处理规则

如果 10 分钟内没有任何批次状态更新，应视为疑似卡住。此时需要输出：

1. 当前各模型的 `done/running/pending/blocked` 批次数。
2. 每个模型最后完成的批次。
3. 每个模型建议从哪个批次继续。

恢复时必须保证幂等：已经完成的批次不重复写入、不重复汇总。

## 模型质量漂移处理规则

在 pilot gate 和长批次审核过程中，都要关注模型是否过严或过松。

触发告警的情况包括：

- 删除率低于 `5%`，可能过松。
- 删除率高于 `45%`，可能过严。
- 某模型删除率与同组模型中位数相差超过 `15` 个百分点。
- 删除理由反复不符合审题提示词或错误分类定义。

如果触发质量告警，需要：

1. 暂停该模型后续批次。
2. 展示该模型 5 个保留样例和 5 个删除样例。
3. 说明该模型可能误解了哪条规则。
4. 校准提示词后重新跑 pilot。
5. 只有重新通过 pilot 后，才继续全量审核。

## 与用户沟通时的汇报顺序

向用户汇报时，按以下顺序输出：

1. 当前轮次的总体进度。
2. 是否有卡住、失败或质量漂移风险。
3. 删除题目的一级错误类型统计。
4. 删除题目的二级错误类型统计。
5. 每个二级错误类型的 2 个典型例子。
6. 下一步动作，以及是否需要用户介入。

如果当前存在卡住或质量漂移，还要追加：

7. 每个模型应从哪个批次继续。
8. 每个模型是否需要重新 pilot。

## 可用脚本

辅助脚本位于 `scripts/` 目录，用于生成 dashboard、导出轮次产物和整理细分统计。使用脚本时必须遵守本文件中的轮次输出规则和字段规则。
